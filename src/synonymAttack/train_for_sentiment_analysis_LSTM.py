...
def train():
    ...
    path_to_sentiment_model: str = (
        "models/sentimentAnalysis/lstm_sentiment.pt"
    )
    ...
    for epoch in range(4):
        ...
        for i, (inputs, labels) in enumerate(train_loader):
            ...
            output_probs, sentiment_h = sentiment_model(
                torch.tensor(synonym_inputs).to(device), sentiment_h
            )

            output_probs = output_probs.to(device)
    ...
    save_model(
        synonym_model,
        "models/synonymAttack/synonym_attack_lstm_sentiment.pt",
    )
